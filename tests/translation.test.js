...
describe('the translation file', () => {
    ...
    const check_translation_against_base = function(base, trans, path, logToConsole) {
        Object.keys(base).forEach(key => {
            ...
            if (typeof base[key] === 'string') {
                ...
                check_translation_against_base(base[key], trans[key], tempPath, logToConsole);
            }
        });
    };
    ...
    it('should contain translations for every key', () => {
        ...
        for (let lang of langs) {
            ...
            check_translation_against_base(base, translation, '', true); // Checking the base translation against all available translation
            expect(failures.length).toEqual(0);
        }
        for (let lang of langs) {
            let translation = require(`../src/locales/${lang}/translation.json`);
            check_translation_against_base(translation, base, '', true); // Checking all available translation against the base translation
            expect(failures.length).toEqual(0);
        }
    });
    ...
    it('should work with any dictionnary', () => {
        ...
        check_translation_against_base(dict_base, dict_translation, '', false);
        expect(failures.length).toEqual(0); // All base keys appear in translation

        check_translation_against_base(dict_translation, dict_base, '', false);
